...
def proxy(request):
    ...
    if cmd == 'login':
        ...
        return JSONResponse({'name': 'Xuxu na Feira', 'sign_url': 'http://www.google.com.br'})

    elif cmd == 'check_time':
        return JSONResponse({'logout': False, 'clean_apps': ['pidgin', 'rhythmbox'], 'error': None, 'full_name': "Junão", 'time': 60, 'after_action':0, 'http_proxy': proxy})
